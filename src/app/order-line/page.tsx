...
export default function OrderLinePage() {
  ...
  function handlePlaceOrder() {
    const result = placeOrder(paymentMethod);
    if (result) {
      setDonation(true);
    }
  }
          ...
          <div className="relative mb-8">
            <div ref={queueRef} className="flex gap-4 overflow-x-auto pb-1 scroll-smooth">
              ...
              {filteredOrders.map((order) => (
                <button
                  key={order.id}
                  onClick={() => loadOrderIntoCart(order.id)}
        ...
        <div className="hidden w-[400px] shrink-0 flex-col border-l border-neutral-200 bg-white p-5 lg:flex overflow-y-auto">
          <div className="mb-4 flex items-start justify-between">
            <div>
              ...
              <p className="mt-0.5 text-sm text-neutral-400">
                {cart.editingOrderId
                  ? `Order #${orders.find((o) => o.id === cart.editingOrderId)?.orderNumber}`
                  : "New Order"}
                {" · "}
            ...
            <div className="mb-4 space-y-3 rounded-xl border border-neutral-200 bg-neutral-50 p-3">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setCartTable(t?.id ?? null, t?.number ?? null, cart.guests);
                  }}
              ...
              <div>
                ...
                <input
                  ...
                  min={1}
                  value={cart.guests}
                  onChange={(e) => setCartTable(cart.tableId, cart.tableNumber, Math.max(1, Number(e.target.value)))}
                  className="w-full rounded-lg border border-neutral-200 px-3 py-2 text-sm outline-none focus:border-teal-500"
                />
              </div>
          ...
          {cart.editingOrderId && (
            <div className="mt-4 flex items-center justify-between rounded-xl bg-neutral-50 px-3 py-2 text-sm">
              ...
              <StatusStepper
                status={orders.find((o) => o.id === cart.editingOrderId)!.status}
                onAdvance={(next) => setOrderStatus(cart.editingOrderId!, next)}
              />
            </div>
          )}
...
function MenuTab({
  ...
  active: boolean;
  icon: Parameters<typeof CategoryIconView>[0]["icon"];
  label: string;
  count: number;
  onClick: () => void;
}) {

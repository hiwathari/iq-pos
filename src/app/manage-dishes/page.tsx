...
import type { CategoryIcon, Dish } from "@/lib/types";
import { LayoutGrid, List, Plus, Search, SlidersHorizontal, MoreVertical, Pencil, Trash2 } from "lucide-react";

export default function ManageDishesPage() {
  const { categories, dishes, addCategory, addDish, updateDish, deleteDish } = useStore();
  ...
  function handleSaveDish(values: { name: string; categoryId: string; price: number; emoji: string; description: string }) {
    if (editingDish) {
      updateDish(editingDish.id, values);
    } else {
      addDish({ ...values, color: "#DCEEE8" });
    }
                ...
                <tbody className="divide-y divide-neutral-100">
                  {filteredDishes.map((dish) => (
                    <tr key={dish.id} className="hover:bg-neutral-50/60">
                      ...
                      <td className="px-5 py-3 font-semibold text-neutral-800">${dish.price.toFixed(2)}</td>
...
function DishCard({
  ...
  return (
    <div className="relative flex flex-col rounded-2xl border border-neutral-200 bg-white p-4 hover:shadow-md transition-shadow">
      ...
      <div className="font-semibold text-neutral-800">${dish.price.toFixed(2)}</div>
    </div>
  );
}
